...
export function drawMap(handleMapClick, handleMarkerDrag) {
  ...
  map.on('load', function() {
    ...
    map.on('mouseleave', 'start', function() {
      map.setPaintProperty('start', 'circle-color', '#19b566');
      canvas.style.cursor = '';
    ...
    map.on('mouseenter', 'end', function() {
      map.setPaintProperty('end', 'circle-color', '#3bb2d0');
      canvas.style.cursor = 'move';
      isCursorOverPoint = true;
      map.dragPan.disable();
    });
    ...
    map.on('mouseleave', 'end', function() {
      map.setPaintProperty('end', 'circle-color', '#cf3043');
      canvas.style.cursor = '';
      isCursorOverPoint = false;
      map.dragPan.enable();
    });
  ...
  .then(geojson => {
    ...
    map.on('click', 'lockers', function (e) {
    ...
    map.on('mouseenter', 'lockers', function() {
      map.setPaintProperty('lockers', 'circle-color', '#f55ef3');
      canvas.style.cursor = 'pointer';
    });

    map.on('mouseleave', 'lockers', function() {
  ...
  // BTWD Stations layer
  const btwdStationsDatasetID = 'bikesy/cjgcqb96q00h3f6moizqvdzkw';
  fetch(`https://api.mapbox.com/datasets/v1/${btwdStationsDatasetID}/features?access_token=${config.mapboxAccessToken}`)
  .then(response => response.json())
  .then(geojson => {
    map.addSource('btwdStations', {
      type: 'geojson',
      data: geojson
    });

    map.on('click', 'btwdStations', function (e) {
      console.log(e.features[0])
      var coordinates = e.features[0].geometry.coordinates.slice();
      var description = '<div><b>' + e.features[0].properties.Name + '</b></div>';
      description += '<div>Business/Organization: ' + e.features[0].properties.Business__ + '</div>';
      description += '<div>City: ' + e.features[0].properties.City + '</div>';
      description += '<div>Time: ' + e.features[0].properties.Time + '</div>';

      if (e.features[0].properties.Features !== 'null') {
        description += '<div>Features: ' + e.features[0].properties.Features + '</div>';
      }
    ...
    map.on('mouseenter', 'btwdStations', function() {
      map.setPaintProperty('btwdStations', 'circle-color', '#4955f1');
      canvas.style.cursor = 'pointer';
    });

    map.on('mouseleave', 'btwdStations', function() {
      map.setPaintProperty('btwdStations', 'circle-color', '#1c2bf4');
      canvas.style.cursor = '';
    });
  });
}
