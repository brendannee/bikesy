...
export function cleanElevationProfile(elevationProfile) {
  ...
  for (const [index, point] of elevationProfile.entries()) {
    ...
    const previousPoint = profile[profile.length - 1]
    const nextPoint = elevationProfile.slice(index).find(p => p.elevation !== -1)
    ...
    const newElevation = previousPoint.elevation + ((nextPoint.elevation - previousPoint.elevation) * (point.distance - previousPoint.distance) / (nextPoint.distance - previousPoint.distance))
    profile.push({
      ...point,
